...
import { Table, ExternalLink, Copy } from "lucide-react";
...
const Step3GoogleSheets = ({ questions }: Step3GoogleSheetsProps) => {
  const generateSheetText = () => {
    ...
    let text = "SOP Title\tQuestion\tOption A\tOption B\tOption C\tOption D\tCorrect Answer\n";
    
    questions.forEach((q, index) => {
      const correctLetter = String.fromCharCode(65 + q.correctAnswer);
      text += `Leave Policy\t${q.question}\t${q.options[0]}\t${q.options[1]}\t${q.options[2]}\t${q.options[3]}\tOption ${correctLetter}\n`;
    });
                ...
                {questions.length > 0 ? (
                  questions.map((q, i) => (
                    <tr key={i} className="border-b border-gray-200">
                      <td className="py-2 px-3 text-gray-800">Leave Policy</td>
